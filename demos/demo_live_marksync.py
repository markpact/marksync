#!/usr/bin/env python3
"""
Live marksync demo: prompt -> generate -> parse -> validate -> PDF

Real-time console output + progressive PDF generation.
Each step adds a page to the PDF as it completes.

Usage:
    python demos/demo_live_marksync.py              # interactive menu
    python demos/demo_live_marksync.py --prompt "Build a todo API"
    python demos/demo_live_marksync.py --example todo-api
    python demos/demo_live_marksync.py --list       # list available prompts
"""
...
class LivePDF(FPDF):
    ...
    def add_step_page(self, title, subtitle, left_lines, right_lines, checks, highlight_kind=None):
        ...
        y_start = 22
        total_w = self.W - 3 * self.M
        left_w = total_w * 0.70  # 70% dla lewej kolumny
        right_w = total_w * 0.30  # 30% dla prawej kolumny
        panel_h = self.H - y_start - self.M - 2

        # LEFT panel (70%)
        self._draw_panel(self.M, y_start, left_w, panel_h, "README.md", PC_LEFT_BG, PC_BLUE)
        self._render_mono(self.M, y_start + 9, left_w, y_start + panel_h - 2, left_lines, highlight_kind)

        # RIGHT panel (30%)
        rx = self.M + left_w + self.M
        checks_h = 6 + len(checks) * 6.5 if checks else 0
        self._draw_panel(rx, y_start, right_w, panel_h, "WALIDACJA + OCZEKIWANIA", PC_RIGHT_BG, PC_PURPLE)
        ey = self._render_right(rx, y_start + 9, right_w, y_start + panel_h - checks_h, right_lines)

        # Checks at bottom
        checks_y = max(ey + 2, y_start + panel_h - checks_h)
        self._render_checks(rx, checks_y, right_w, checks)
...
def run_live(prompt_key: str, prompt: str, model: str = None):
    ...
    print()
    info("  Meta-pipeline: human -> marksync (script) -> LLM -> contract")
    print()

    if pdf:
        pdf.add_title_page(session)
        # Add meta-pipeline page
        meta_left = [
            "# Meta-Pipeline: Jak powstal ten dokument?",
            "",
            "Zanim powstal ten kontrakt, czlowiek uruchomil marksync:",
            "",
            "```yaml markpact:orchestration",
            "# Meta-pipeline generowania kontraktu",
            "pipeline:",
            "  name: marksync-report-generation",
            "  steps:",
            "    - name: user_request",
            "      actor: human",
            "      config:",
            "        action: run_command",
            "        command: marksync report",
            "",
            "    - name: parse_intent",
            "      actor: script",
            "      config:",
            "        tool: marksync.intent.IntentParser",
            "",
            ...
            "        prompt: user_request",
            "",
            "    - name: enrich_orchestration",
            "      actor: script",
            "      config:",
            "        tool: marksync.intent.YAMLGenerator",
            "",
            "    - name: render_pdf",
            "      actor: script",
            "      config:",
            "        tool: marksync.report.ReportPDF",
            "```",
            "",
            "Kazdy dokument marksync jest wynikiem wspolpracy:",
            "  1. HUMAN - definiuje zamysl (prompt)",
            "  2. SCRIPT - orkiestruje proces (marksync)",
            "  3. LLM - generuje kontrakt",
            "  4. SCRIPT - wzbogaca + renderuje PDF",
        ]
        meta_right = [
            "[Meta-Pipeline]",
            "",
            "Aktorzy:",
            "  human  - uruchamia marksync",
            "  script - marksync CLI",
            "  llm    - generuje kontrakt",
            "  script - renderuje PDF",
            "",
            "Przepływ:",
            "  human -> marksync report",
            "  marksync -> IntentParser",
            "  marksync -> LLM (prompt)",
            "  LLM -> README.md",
            "  marksync -> YAMLGenerator",
            "  marksync -> ReportPDF",
            "  ReportPDF -> PDF output",
            "",
            "Ten dokument to meta-refleksja:",
            "pokazuje proces wlasnego",
            "powstania.",
        ]
        meta_checks = [
            ("human uruchomil marksync", "pass", "marksync report"),
            ("script parsuje intent", "pass", "IntentParser"),
            ("LLM generuje kontrakt", "pass", config.model),
            ("script wzbogaca YAML", "pass", "YAMLGenerator"),
            ("script renderuje PDF", "pass", "ReportPDF"),
        ]
        pdf.add_step_page(
            "0. Meta-Pipeline",
            "Jak powstal ten dokument? human -> marksync -> LLM -> PDF",
            meta_left, meta_right, meta_checks, "orchestration"
        )
    ...
    # ── Step 1b: Enrich with marksync orchestration
    hdr("KROK 1b: Wzbogacanie kontraktu -- marksync orchestration")

    try:
        from marksync.intent.parser import IntentParser, ProcessIntent
        from marksync.intent.yaml_generator import YAMLGenerator
        import json as _json

        t0b = time.time()
        # Parse intent from prompt (heuristic, no LLM needed)
        intent = ProcessIntent.from_prompt(prompt)
        step(f"{CYAN}>>>{NC}", f"service_type={BOLD}{intent.service_type}{NC}, actors={intent.actors}")

        # Generate orchestration YAML
        yaml_gen = YAMLGenerator()
        yaml_blocks = yaml_gen.generate(intent)

        # Check if orchestration already in README (LLM might have generated it)
        has_orch = "markpact:orchestration" in readme
        has_state = "markpact:state" in readme
        has_pipeline = "markpact:pipeline" in readme

        injected = []
        inject_text = ""

        if not has_orch and "markpact:orchestration" in yaml_blocks:
            inject_text += f"\n```yaml markpact:orchestration\n{yaml_blocks['markpact:orchestration']}```\n"
            injected.append("orchestration")

        if not has_pipeline and "markpact:pipeline" in yaml_blocks:
            inject_text += f"\n```yaml markpact:pipeline\n{yaml_blocks['markpact:pipeline']}```\n"
            injected.append("pipeline")

        if not has_state:
            state_json = _json.dumps({
                "phase": "init", "health": None,
        ...
        # Add log block
        if "markpact:log" not in readme:
            ts_now = time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())
            inject_text += f"\n```text markpact:log\n[{ts_now}] CONTRACT_CREATED: name={intent.name}\n```\n"
            injected.append("log")

        if inject_text:
            # Insert orchestration blocks right after the first --- or after the header section
            # Find insertion point: after description, before first markpact block
            insert_idx = readme.find("\n```")
            if insert_idx > 0:
                readme = readme[:insert_idx] + "\n" + inject_text + readme[insert_idx:]
            else:
                readme += inject_text
        ...
        dur_enrich = (time.time() - t0b) * 1000

        if injected:
            ok(f"Wzbogacono o {len(injected)} blokow", ", ".join(f"markpact:{b}" for b in injected))
            session.add_step("marksync enrich", "pass",
                           f"+{len(injected)} blocks: {', '.join(injected)}", dur_enrich)
        else:
            ok("Wszystkie bloki juz obecne (LLM wygenerował orchestration)")
            session.add_step("marksync enrich", "pass", "all blocks present", dur_enrich)

    except ImportError:
        info("  marksync nie zainstalowane -- pomijam wzbogacanie")
        session.add_step("marksync enrich", "skip", "marksync not installed")
    except Exception as e:
        fail(f"Wzbogacanie: {e}")
        session.add_step("marksync enrich", "fail", str(e)[:80])
    ...
    if pdf:
        right_enrich = [
            ...
            "  markpact:orchestration -- pipeline kroków",
            "  markpact:pipeline -- definicja steps",
            "  markpact:state -- stan procesu",
            "  markpact:log -- historia zdarzeń",
            "",
            f"  service_type: {intent.service_type if 'intent' in dir() else '?'}",
            f"  actors: {', '.join(intent.actors) if 'intent' in dir() else '?'}",
        ]
        enrich_checks = [(f"markpact:{b} injected", "pass", "") for b in (injected if 'injected' in dir() else [])]
        if not enrich_checks:
            enrich_checks = [("All blocks present", "pass", "LLM generated orchestration")]
        pdf.add_step_page("1b. marksync Orchestration",
                         "IntentParser + YAMLGenerator",
                         readme.split("\n")[:50], right_enrich, enrich_checks, "orchestration")
    ...
    required_kinds = ["orchestration", "state", "deps", "file", "run"]
    optional_kinds = ["pipeline", "log", "test", "target"]
    ...
    if pdf:
        ...
        pdf_path = session.output_dir / f"marksync_live_{prompt_key.replace('-', '_')}.pdf"
...
def main():
    parser = argparse.ArgumentParser(
        description="marksync -- Live Contract Generator with PDF output")
